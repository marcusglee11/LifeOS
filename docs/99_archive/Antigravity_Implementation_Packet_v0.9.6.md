# Antigravity Implementation Packet v0.9.6
...
### 0.2 Implementation Packet Versioning
  ...
  * This document: **Antigravity Implementation Packet v0.9.6**
  ...
  * Git tagging convention:
      ...
      * `impl/project-builder-v0.9.6` – tag on commit where this packet and the compliant implementation are merged to `main`.
...
3.  Result in a new version suffix (e.g. v0.9.7) and corresponding Git tag.
...
│   ├── config/
│   │   ├── __init__.py
│   │   └── settings.py           # MAX_REPAIRS_PER_TASK, timeouts, etc.
│   ├── database/
│   │   ├── __init__.py
...
└── docs/
    ...
    ├── Implementation_Packet_v0_9_6.md
    └── CHANGELOG.md
```
...
#### Definitions
  ...
  * `max_pending_total`: integer (default 12)
  * `resume_threshold`: integer (default 6)

#### Pending Count

Pending = tasks with status IN ('pending','repair\_retry') + messages routed but not executed.

#### Pause Logic

When pending \> max\_pending\_total:

1.  Mission status MUST transition to "paused\_error".
2.  Future planner outputs MUST be rejected with `error_type="backpressure_active"`.
3.  No new tasks may be created.

#### Resume Logic

Mission may leave paused\_error ONLY when pending \< resume\_threshold.
When resuming:

  * Maintain existing repair\_context.
  * Maintain repair\_attempt values.
  * Do not reset started\_at.
...
Implement:
  ...
  * `check_and_apply_backpressure()`
  * `resume_mission_if_ready()`
    Each MUST run inside a single `BEGIN IMMEDIATE` transaction.
...
### 8.3 Budget Transaction
  ...
  * `test_budget_txn.py`:
      ...
      * Over-spend repair budget on repair attempts → fails.
      * Non-repair tasks do not touch `repair_budget_spent_usd`.
...
### 8.9 Additional Tests

  * `test_reclaim_malformed_locked_by`:
      * locked\_by="garbage"
      * Expect: skip reclaim, timeline event logged.
...
Deliver:
  ...
  * `fsm.py`, `budget_txn.py`, `reclaim.py`, `routing.py`
  * `tokenizer.py`, `injection.py`, `truncation.py`
  * Tests: budget\_txn, fsm, repair\_context, tokenizer\_replay
...
### 12.2 Tagging & Releases
  ...
  * Tag the implementation commit (first stable version matching this packet) as:
      * `impl/project-builder-v0.9.6`

Subsequent compatible changes:

  * Bug fixes without spec change:
      * `impl/project-builder-v0.9.7`, etc.
...
1.  Reference:
      ...
      * Implementation packet version: `Packet v0.9.6`
2.  Include:
      * Summary of changes.
...
- Added Backpressure Control logic (max_pending=12).
...
### Notes
- Aligned with GPTCOO v1.1 Project Builder Spec v0.9 and Implementation Packet v0.9.6.
```

### 12.5 Release Checklist

Before tagging `impl/project-builder-v0.9.6`:
...
5.  Implementation Packet path `docs/Implementation_Packet_v0_9_6.md` committed.
...
```bash
git tag impl/project-builder-v0.9.6
git push origin impl/project-builder-v0.9.6
```
