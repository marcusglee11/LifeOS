...
# File: 02_protocols/RPPV_Integration_Notes.md

# RPPV Integration Notes
...
### Canonical Packet Assembly Pipeline

**File**: `scripts/packaging/build_return_packet.py`

**Line References**:

- L181-210: Validator integration (calls `validate_return_packet_preflight.py`)
- L198: Comment explicitly mentioning validator requirement
- L205-206: Subprocess call to validator

### Existing Validator

**Discovery**: The repository already has a return packet validator at:

- `scripts/packaging/validate_return_packet_preflight.py`

This is a **separate, pre-existing validator** with its own rule set (different from RPC v1.0).

---

## Integration Status: DOCUMENTED (Not Wired)

Per the fail-closed requirement in E5/E6, I discovered that:

1. **Multiple validators exist**:
   - Pre-existing: `scripts/packaging/validate_return_packet_preflight.py` (different rule set)
   - New RPC v1.0: `scripts/validators/rppv.py` (this implementation)

2. **Existing pipeline already validated**:
   - `build_return_packet.py` already calls the pre-existing validator
   - Unclear if this validator is deprecated or canonical

3. **No single canonical integration point**:
   - `build_return_packet.py` is used by `build_closure_bundle.py` (L245)
   - Unclear if this is the ONLY packet assembly path
...
### Option A: Replace Pre-Existing Validator

- Rename `validate_return_packet_preflight.py` → `validate_return_packet_preflight.deprecated.py`
- Update `build_return_packet.py` L205 to call `scripts.validators.rppv`
- **Risk**: May break existing workflows if old validator is still in use

### Option B: Run Both Validators

- Keep existing validator
- Add second call to `rppv.py` in `build_return_packet.py`
- **Risk**: Dual validation may have conflicting rules

### Option C: Gradual Migration

- Create `build_return_packet_v2.py` using RPC v1.0
- Deprecate old builder over time
- **Risk**: Maintains two pipelines temporarily

---

## Manual Integration Example

To wire RPPV as a hard gate in `build_return_packet.py`, modify around L205:

```python
# Before zipping final packet (around L200):
import subprocess
import sys

# Run RPPV
rppv_result = subprocess.run(
    [sys.executable, "-m", "scripts.validators.rppv",
     "--packet-dir", str(packet_folder),
     "--class", "exit_blocker"],
    capture_output=True,
    text=True
)
...
if rppv_result.returncode != 0:
    # FAIL-CLOSED: Write BLOCKED.md
    blocked_path = packet_folder / "BLOCKED.md"
    blocked_path.write_text(
        f"# BLOCKED: RPC v1.0 Validation Failed\n\n"
        f"```\n{rppv_result.stdout}\n```\n"
    )
    print(f"BLOCKED: Packet failed RPC v1.0 validation", file=sys.stderr)
    print(rppv_result.stdout, file=sys.stderr)
    sys.exit(1)
```

**File:Line Reference**: `scripts/packaging/build_return_packet.py:200-220` (suggested insertion point)

---

## Testing RPPV Manually

### Validate Directory

```bash
python -m scripts.validators.rppv --packet-dir <path> --class exit_blocker
```

### Validate Zip

```bash
python -m scripts.validators.rppv --zip <path> --class exit_blocker
```

### JSON Output

```bash
python -m scripts.validators.rppv --packet-dir <path> --class exit_blocker --json
```
...
Unit tests implemented in `tests/test_rppv.py`:

- T-RPC-PASS: Valid packet passes
- T-RPC-00: Missing file detection
- T-RPC-01: Empty patch detection
- T-RPC-02: Out-of-scope path detection
- T-RPC-03: Commit hash mismatch detection
- T-RPC-04: Manifest hash mismatch detection
- T-RPC-05: Certification/result truthfulness
- T-RPC-06: Truncation detection

**Note**: Tests require fixtures to be generated (see `tests/fixtures/rpc/`).

---

## Evidence

**Grep Search Results** (discovery command):

```
grep -rn "Return_Packet" scripts/
```

**Key Findings**:

- `build_return_packet.py`: Main assembly script
- `validate_return_packet_preflight.py`: Existing validator
- `build_closure_bundle.py`: Calls packet builder

**Integration Status**: BLOCKED pending user direction on validator consolidation strategy.


---

# File: 02_protocols/Return_Packet_Contract_RPC_v1.0.md
