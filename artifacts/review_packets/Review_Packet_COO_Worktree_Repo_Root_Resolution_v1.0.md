# Review Packet: COO Worktree Repo Root Resolution v1.0

## Scope
Resolved `coo` startup failures caused by repository root mis-resolution when invoked outside the LifeOS repo (or from a different git repo), which led to missing internal runtime tool paths during `coo start` flows.

## Root Cause
`runtime/tools/coo_worktree.sh` previously initialized:
- `BUILD_REPO="$(git rev-parse --show-toplevel)"`

This depended on caller cwd instead of the script's own location. As a result:
- running `coo` from non-git directories failed immediately (`fatal: not a git repository`), and
- running from an unrelated git repo could resolve `BUILD_REPO` incorrectly and break internal `runtime/tools/*` references.

## Changes Implemented
1. `runtime/tools/coo_worktree.sh`
- Added symlink-safe script source resolution (`readlink -f` fallback-safe).
- Added `SCRIPT_DIR` and `SCRIPT_REPO` derivation from the script path.
- Added `resolve_build_repo()` with priority:
  1. git top-level from `SCRIPT_REPO`
  2. git top-level from caller cwd (fallback)
  3. static `SCRIPT_REPO` if expected script path exists
- Added `LIFEOS_BUILD_REPO` override support.
- Added fail-closed validation that `BUILD_REPO` contains `runtime/tools/coo_worktree.sh`.

2. `runtime/tests/test_coo_worktree_breakglass.py`
- Extended `_run_start()` helper to accept an explicit `cwd`.
- Added regression test `test_start_resolves_repo_when_invoked_outside_repo` to prove `coo_worktree.sh start` succeeds when called from outside repo cwd.
...
Manual repro check:
- `cd /tmp && /home/cabra/.local/bin/coo path` now resolves successfully to the LifeOS training worktree path instead of failing with git-root errors.
...
def test_start_resolves_repo_when_invoked_outside_repo(tmp_path: Path) -> None:
    ...
    outside_cwd.mkdir()
    proc = _run_start(repo_dir, env, cwd=outside_cwd)
    assert proc.returncode == 0, proc.stderr
