# Review Packet: OpenClaw Model Stability Fix v1.0

## Scope
Stabilized OpenClaw runtime routing and preflight verification after observed session hangs when model routing shifted from `grok-code-fast-1` to Claude Sonnet.

Actions completed:
- Verified `claude-max-api` health endpoint and live chat completion path.
- Confirmed `openai-codex` auth order pin to `openai-codex:default`.
- Corrected main agent routing in user config to policy-compliant, working ladder:
  - `primary`: `openai-codex/gpt-5.3-codex`
  - `fallbacks`: `["google-gemini-cli/gemini-3-flash-preview"]`
- Restarted OpenClaw gateway and re-ran model probe.
- Fixed preflight script crash on policy-failure path due to unbound `OPENCLAW_CONFIG_PATH`.

## Branch
- current working tree (no branch switch requested)

## Commit
- not committed in this packet

## Verification
- `curl -fsS http://127.0.0.1:3456/health` -> `status: ok`
- POST `http://127.0.0.1:3456/v1/chat/completions` with `claude-sonnet-4` and valid JSON payload -> `OK`
- `openclaw models status --json --probe` -> key providers returned `status: ok` (including `openai-codex:default`, `claude-max`, `github-copilot`, `copilot-proxy`, `google-gemini-cli`)
- `bash runtime/tools/openclaw_models_preflight.sh` -> `PASS models_preflight=true reason=ok`
