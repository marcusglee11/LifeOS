# Review Packet: COO Launcher RepoRoot Hotfix v1.0

## Mission Scope
Fix `coo` launcher behavior when invoked from home directory (`~`) so repo root is resolved to the LifeOS repository instead of `/home/cabra`, restoring runtime tool path resolution for `coo start` / `coo tui` flows.

## Reported Failure
Observed by operator:
- `coo tui` printed:
  - `BUILD_REPO=/home/cabra`
  - missing internal tool scripts under `runtime/tools/...`
- startup then failed closed.

## Root Cause
`runtime/tools/coo_worktree.sh` used script-relative resolution from `BASH_SOURCE[0]` without canonicalizing symlinks. Since `coo` is launched via symlink (`~/.local/bin/coo`), path resolution could collapse to `/home/cabra` in specific invocation contexts.

## Implemented Fix
Updated `runtime/tools/coo_worktree.sh` to:
1. Canonicalize script path using `readlink -f` when available.
2. Resolve script directory/repo from canonicalized source path.
3. Resolve `BUILD_REPO` through deterministic fallback helper.
4. Fail closed if resolved repo does not contain `runtime/tools/coo_worktree.sh`.
...
Results:
- `coo path` => `/mnt/c/Users/cabra/Projects/LifeOS__wt_coo_training`
- `coo ensure` prints `BUILD_REPO=/mnt/c/Users/cabra/Projects/LifeOS`

### Startup surface check
Command:
- `cd ~ && coo start`

Result:
- startup proceeds to gateway/model preflight phase using correct repo path.
- remaining failure in this sandbox is environment confinement/permissions, not missing tool paths.

### Targeted regression test
Command:
- `pytest -q runtime/tests/test_coo_worktree_breakglass.py`
