---
artifact_id: "0cc190e4-a29f-4d8a-aeec-1d1cbccf381e"
artifact_type: "REVIEW_PACKET"
schema_version: "1.0.0"
created_at: "2026-01-25T14:40:00Z"
author: "OpenCode Doc Steward"
version: "1.0"
status: "PENDING_REVIEW"
terminal_outcome: "PASS"
closure_evidence:
  doc_steward_protocol: "docs/01_governance/Document_Steward_Protocol_v1.0.md"
  regen_script: "docs/scripts/generate_strategic_context.py"
  index_doc: "docs/INDEX.md"
---
# Review_Packet_dogfood_v5_v1.0

# Scope Envelope
- **Allowed Paths**: `docs/99_archive/dogfood_v5.md`, `docs/INDEX.md`, `docs/LifeOS_Strategic_Corpus.md`
- **Forbidden Paths**: Top-level directories outside `docs/99_archive`, runtime code, and governance foundations not declared in the mission scope
- **Authority**: Document Steward Protocol v1.0 via `docs/01_governance/Document_Steward_Protocol_v1.0.md` (Doc Steward instructions and AGENTS guidance)

# Summary
Added the GPT-5 dogfooding note to `docs/99_archive`, surfaced it in the archive index, and regenerated the Strategic Corpus so the lightweight context reflects the latest archive entry.

# Issue Catalogue

| Issue ID | Description | Resolution | Status |
|----------|-------------|------------|--------|
| P0.1     | No blocking issues encountered (docs-only update) | N/A | CLOSED |

# Acceptance Criteria

| ID | Criterion | Status | Evidence Pointer | SHA-256 |
|----|-----------|--------|------------------|---------|
| AC1 | `docs/99_archive/dogfood_v5.md` exists with the requested text | PASS | docs/99_archive/dogfood_v5.md | 4fd5ffa142a62c2e37a550a57dfa4104ade2823840bb39cdbb77256034967c4f |
| AC2 | `docs/INDEX.md` timestamp and archive entry now cite the new file | PASS | docs/INDEX.md | ee2c108e47946c8a74c7a57c6eb2528a74365e17923d58727e69f99ebf24bf9e |
| AC3 | `docs/LifeOS_Strategic_Corpus.md` regenerated via `docs/scripts/generate_strategic_context.py` | PASS | docs/LifeOS_Strategic_Corpus.md | da98542ec32b0d2ea199a319701b8e3135e19ed010f7b447f91250d093b0d6e0 |

# Closure Evidence Checklist

| Category | Requirement | Verified |
|----------|-------------|----------|
| **Provenance** | Code commit hash + message | N/A (work not yet committed) |
|  | Docs commit hash + message | N/A (work not yet committed) |
|  | Changed file list (paths) | `docs/99_archive/dogfood_v5.md`, `docs/INDEX.md`, `docs/LifeOS_Strategic_Corpus.md` |
| **Artifacts** | `attempt_ledger.jsonl` | N/A (not generated) |
|  | `CEO_Terminal_Packet.md` | N/A (not generated) |
|  | `Review_Packet_attempt_XXXX.md` | N/A (not generated) |
|  | Closure Bundle + Validator Output | N/A (not generated) |
|  | Docs touched (each path) | See changed files above |
...
| **Governance** | Doc-Steward routing proof | AGENTS.md + `docs/01_governance/Document_Steward_Protocol_v1.0.md` |
|  | Policy/Ruling refs invoked | Document Steward Protocol v1.0 (Doc Steward instructions) |
| **Outcome** | Terminal outcome proof | PASS via this review packet and updated docs |

# Non-Goals

- Not modifying runtime code or governance foundations outside the permitted archive entry.
- Not issuing a code commit as part of this docs-only mission.
- Not generating additional artifact bundles beyond the regenerated Strategic Corpus.
...
## Flattened Code
### File: `docs/99_archive/dogfood_v5.md`

````markdown
Feature: GPT-5 Dogfooding
Status: Verified
````
...
````markdown
# LifeOS Strategic Corpus [Last Updated: 2026-01-25T14:30:00Z (Doc Steward)]
...
- `[dogfood_v5.md](./99_archive/dogfood_v5.md)` — GPT-5 dogfooding verification note
...
# File: 01_governance/OptionC_OpenAI.md


# Option C: OpenAI via ChatGPT Subscription

LifeOS workflows invoking OpenCode can optionally use OpenAI models authenticated via a ChatGPT Plus/Pro subscription (Opencode Plugin), instead of the default Zen/OpenRouter keys.

## 1. Prerequisites (One-Time Bootstrap)

To enable this path, you must authenticate the `opencode-openai-codex-auth` plugin on the machine where LifeOS runs (e.g., your local machine).

1. **Install/Update OpenCode**: Ensure you have the latest version.
2. **Authenticate**:
    Run the following command in your terminal:

    ```bash
    opencode auth login opencode-openai-codex-auth
    ```

    (Or simply `opencode auth login` and select the provider from the list)

    Follow the browser flow to link your ChatGPT subscription.

    *Result*: Credentials will be stored in `~/.local/share/opencode/auth.json`.

## 2. Configuration (How to Select)

To configure LifeOS to use this path for a specific agent (e.g., "builder" or "steward"), update `config/models.yaml`:
...
agents:
  steward:
    ...
    model: "openai/gpt-4o"  # Must use 'openai/' prefix for correct CLI routing
    endpoint: "https://opencode.ai/plugin/openai" # (Optional/Symbolic)
    api_key_env: "" # No API Key needed (uses Plugin Auth)
```

## 3. Behavior

- **Routing**: When `provider` is set to `opencode-openai` (or contains "opencode-openai"), the `OpenCodeClient` bypasses direct REST calls to Zen/OpenRouter and delegates execution to the local `opencode run` CLI.
- **Fail-Closed**: If the plugin is not authenticated, the CLI call will fail. LifeOS will catch this failure and attempt any configured fallbacks (e.g., to OpenRouter) defined in the `fallback` list.
- **No-Hang Guarantee**: The system uses a timeout on the CLI call to ensure it does not hang pending authentication prompts.

## 4. Usage in LifeOS

This option interacts with **Phase 4 Autonomy** via the `OpenCodeClient` execution path. It is NOT available for internal direct-HTTP tools that do not use the OpenCode wrapper.
...
> [!IMPORTANT]
> This section reflects the available evidence for this documentation package.

| ID | Item | Status | Evidence |
|----|------|--------|----------|
| **E1** | ZIP Hash Integrity | N/A | Not applicable (no ZIP produced) |
| **E2** | Packet Hash Citation Matches ZIP | N/A | Not generated |
| **E3** | Bundle Layout Matches Contract | N/A | No bundle created |
| **E4** | Canonical Protocol Doc Reference | N/A | Document Steward instructions cited above |
| **E5** | Provenance Hygiene | N/A | Work pending commit |
| **E6** | Audit-Grade Manifest | N/A | Not produced |
