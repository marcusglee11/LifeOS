# Review Packet: OpenClaw P0.3 Hardening v1.0

## Mission
Validate `coo -> openclaw` execution path and harden OpenClaw P0 surfaces for daily-safe operation.

## Canonical Execution Command
`coo openclaw -- <openclaw-args>`

## Guarantees
- Worktree isolation: forces execution under `../LifeOS__wt_coo_training`
- Deterministic state path: `OPENCLAW_STATE_DIR=$HOME/.openclaw`
- Deterministic config path: `OPENCLAW_CONFIG_PATH=$HOME/.openclaw/openclaw.json`
- Hardened config surface: elevated tools disabled, browser control disabled, owner-only command control gate, constrained model ladders

## Evidence Index
- P0.0 baseline probes (host): `artifacts/evidence/openclaw/p0_hardening/20260210T072856Z/p0_0_probes_pre_host.md`
- P0 post-change probes: `artifacts/evidence/openclaw/p0_hardening/20260210T074010Z/p0_postchange_probes_v2.md`
- Receipts bundle (redacted): `artifacts/evidence/openclaw/receipts/Receipt_Bundle_OpenClaw_P0_20260210T074618Z.md`
- OpenClaw config redacted diff: `artifacts/evidence/openclaw/p0_hardening/20260210T074010Z/openclaw_config_redacted.diff`
- OpenClaw config redacted post state: `artifacts/evidence/openclaw/p0_hardening/20260210T074010Z/openclaw_config_post_redacted.json`

## Outcome by Priority
- P0.0: Completed. Direct vs `coo` probes captured verbatim; wrapper/shim paths verified.
- P0.1: Completed. `coo openclaw` is canonical and deterministic. Profile split was removed from default path.
- P0.2: Completed. `openclaw-sandbox:bookworm-slim` exists on host (`docker image inspect` verified). `sandbox explain` reports active sandbox mode.
- P0.3: Completed. `tools.elevated: disabled` and `browser control: disabled` confirmed in `coo openclaw -- security audit --deep`.
- P0.4: Completed with owner-only gating. Daily and review ladders configured with short auto-fallbacks; manual-only providers are configured but not in fallbacks. Owner gating enforced via `commands.ownerAllowFrom`.
- P0.5: Completed. Redacted receipts script created: `runtime/tools/openclaw_receipts_bundle.sh`.

## Residual Notes
- `openrouter/pony-alpha` currently probes as unknown model in this environment; it remains configured as manual-only and excluded from fallbacks.
- Existing transcript gaps reported by doctor were preserved; no legacy state was deleted.

## Patch Summary
- Updated `runtime/tools/coo_worktree.sh` to route OpenClaw through deterministic state/config and default to non-profile execution.
- Added `runtime/tools/openclaw_receipts_bundle.sh` for deterministic redacted receipts.
- Added `runtime/tools/OPENCLAW_COO_RUNBOOK.md` documenting canonical command and safety guarantees.
- Hardened host config `~/.openclaw/openclaw.json` (redacted snapshot/diff in evidence index).
...
Use `coo openclaw -- <openclaw-args>` for all COO operations.

## Guarantees Provided by `coo openclaw`

- Forces execution from COO training worktree: `../LifeOS__wt_coo_training`
- Uses deterministic OpenClaw state path: `OPENCLAW_STATE_DIR=$HOME/.openclaw`
- Uses deterministic OpenClaw config path: `OPENCLAW_CONFIG_PATH=$HOME/.openclaw/openclaw.json`
- Uses hardened defaults from the shared canonical config (sandbox, tools, model policy)

## Why Direct `openclaw` Is Not Canonical for COO

Direct `openclaw ...` does not enforce worktree isolation. COO operations can drift into the main workspace.

## State Migration / Preservation

State is unified at `~/.openclaw`.

Backups created during P0.3 hardening:

- `~/.openclaw/openclaw.json.bak.p0-hardening-<timestamp>`
- `~/.openclaw/openclaw.json.bak.p0-hardening-fix-<timestamp>`
- `~/.openclaw/openclaw.json.bak.p0-hardening-clean-<timestamp>`
- `~/.openclaw/openclaw.json.bak.p0-hardening-main-deny-<timestamp>`

No legacy state directories were deleted.

## Operator Quick Checks

```bash
coo openclaw -- security audit --deep
coo openclaw -- models status --probe
coo openclaw -- sandbox explain
...
This emits a redacted, deterministic markdown report under:

- `artifacts/evidence/openclaw/receipts/`
```

### File: ~/.openclaw/openclaw.json (redacted)

```json
{
  "meta": {
    "lastTouchedVersion": "2026.2.6-3",
    "lastTouchedAt": "2026-02-10T06:31:05.890Z"
  },
  "wizard": {
    "lastRunAt": "2026-02-10T06:31:05.885Z",
  ...
  "auth": {
    "profiles": {
      ...
      "google-antigravity:garfieldlee11@gmail.com": {
        "provider": "google-antigravity",
        "mode": "oauth"
      },
      "google-gemini-cli:garfieldlee11@gmail.com": {
        "provider": "google-gemini-cli",
        "mode": "oauth"
      },
      "anthropic:cabramattacowboy": {
        "provider": "anthropic",
        "mode": "token"
      }
    },
