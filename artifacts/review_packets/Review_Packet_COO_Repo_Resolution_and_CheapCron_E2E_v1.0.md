# Review Packet: COO Repo Resolution and Cheap Cron E2E v1.0

## Mission Scope
Restore deterministic `coo` repo-root resolution (including symlink entrypoint usage), then validate cheap-model cron behavior end-to-end through the `coo openclaw` path.

## Root Cause
`runtime/tools/coo_worktree.sh` resolved `BUILD_REPO` from `BASH_SOURCE[0]` without canonicalizing symlinks. When invoked via `/home/cabra/.local/bin/coo` symlink, script-relative resolution produced `/home` instead of the LifeOS repo, causing invalid paths and `git` failures.

## Implemented Changes
1. `runtime/tools/coo_worktree.sh`
- Added symlink-safe script path resolution via `readlink -f` fallback-safe logic.
- Added deterministic `resolve_build_repo()` flow:
  - script repo git root
  - current cwd git root fallback
  - static script repo fallback if expected file exists
- Added fail-closed validation that `BUILD_REPO/runtime/tools/coo_worktree.sh` exists.

2. `runtime/tests/test_coo_worktree_breakglass.py`
- Extended `_run_start()` helper to support custom `cwd`.
- Added `test_start_resolves_repo_when_invoked_via_symlink` regression test.
- Pinned `OPENCLAW_BIN` to fixture stub to keep tests hermetic.

3. `runtime/tests/test_coo_worktree_marker_receipt.py`
- Pinned `OPENCLAW_BIN` to fixture stub to avoid host-environment contamination.

## Verification Evidence
### Targeted tests
Command:
`pytest -q runtime/tests/test_coo_worktree_breakglass.py runtime/tests/test_coo_worktree_marker_receipt.py`

Result:
- `6 passed`

### COO repo path restoration
Commands:
- `coo path`
- `coo ensure`

Result:
- `coo path` returns `/mnt/c/Users/cabra/Projects/LifeOS__wt_coo_training`
- `coo ensure` prints `BUILD_REPO=/mnt/c/Users/cabra/Projects/LifeOS` and no git-root failure.

### Cheap cron E2E through `coo openclaw`
Lifecycle command sequence executed (with elevated runtime context for gateway access):
1. Create job without explicit `--model` / `--thinking`
2. Run job immediately (`cron run --expect-final`)
3. Verify persisted job payload and status
4. Remove job

Observed values:
- `COO_E2E_CREATE_MODEL=coo:cheap:cron`
- `COO_E2E_CREATE_THINKING=off`
- `COO_E2E_RUN_RC=0`
- `LAST_STATUS=ok`
- Cleanup confirmed (`removed: true`)

### Policy/guard checks
Commands:
- `python3 runtime/tools/openclaw_cron_delivery_guard.py --json`
- `python3 runtime/tools/openclaw_model_policy_assert.py --json`

Result:
- both return `true` with `0` violations.

## Changed Files
- `runtime/tools/coo_worktree.sh`
- `runtime/tests/test_coo_worktree_breakglass.py`
- `runtime/tests/test_coo_worktree_marker_receipt.py`
...
resolve_build_repo() {
  if git -C "$SCRIPT_REPO" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    git -C "$SCRIPT_REPO" rev-parse --show-toplevel
    return 0
  fi
  ...
  if [ -f "$SCRIPT_REPO/runtime/tools/coo_worktree.sh" ]; then
    printf '%s\n' "$SCRIPT_REPO"
    return 0
  fi

  echo "ERROR: unable to resolve LifeOS repo root for coo_worktree.sh." >&2
  echo "Set LIFEOS_BUILD_REPO to override." >&2
  return 2
}

BUILD_REPO="${LIFEOS_BUILD_REPO:-}"
if [ -z "$BUILD_REPO" ]; then
  BUILD_REPO="$(resolve_build_repo)"
fi
if [ ! -f "$BUILD_REPO/runtime/tools/coo_worktree.sh" ]; then
  echo "ERROR: BUILD_REPO does not contain runtime/tools/coo_worktree.sh: $BUILD_REPO" >&2
  exit 2
fi
...
### FILE: `runtime/tests/test_coo_worktree_breakglass.py`
...
def _run_start(
    repo_dir: Path, env: dict[str, str], *extra: str, cwd: Path | None = None
) -> subprocess.CompletedProcess[str]:
    run_cwd = cwd or repo_dir
...
def test_start_resolves_repo_when_invoked_outside_repo(tmp_path: Path) -> None:
    ...
    outside.mkdir()
    proc = _run_start(repo_dir, env, cwd=outside)
    assert proc.returncode == 0, proc.stderr
    gate = _gate_status(state_dir)
    assert gate["break_glass_used"] is False


def test_start_resolves_repo_when_invoked_via_symlink(tmp_path: Path) -> None:
    repo_dir, env, state_dir = _prepare_repo(tmp_path)
    env["STUB_VERIFY_RC"] = "0"
    env["STUB_GATE_STATUS_JSON"] = json.dumps({"pass": True, "blocking_reasons": []})
    ...
    outside.mkdir()
    entry = outside / "coo"
    entry.symlink_to(repo_dir / "runtime" / "tools" / "coo_worktree.sh")
    ...
    assert proc.returncode == 0, proc.stderr
    assert f"BUILD_REPO={repo_dir}" in proc.stdout
    gate = _gate_status(state_dir)
    assert gate["break_glass_used"] is False

```

### FILE: `runtime/tests/test_coo_worktree_marker_receipt.py`
