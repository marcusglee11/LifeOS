# Review Packet: Index Timestamp - v1.0

## Mission
Updated `docs/INDEX.md` with the latest ISO timestamp and Option C verification status per Doc Steward protocol, then regenerated `docs/LifeOS_Strategic_Corpus.md` to capture the new doc state.

## Changes
- Updated `docs/INDEX.md` timestamp to 2026-01-25T16:00:00Z (Option C Verified).
- Regenerated `docs/LifeOS_Strategic_Corpus.md` after the index change.
...
### docs/INDEX.md
# LifeOS Strategic Corpus [Last Updated: 2026-01-25T16:00:00Z (Option C Verified)]
...
### Smart Model Selection (Dual Config)

For agents that perform complex tasks (like Builder), you can configure a "Dual Model" setup to balance speed and reasoning depth:

```yaml
agents:
  builder:
    provider: "opencode-openai"
    model: "openai/gpt-5.1-codex-mini"       # Default (Fast: ~8s latency)
    reasoning_model: "openai/gpt-5.1-codex-max" # Complex Tasks (Deep: ~60s latency)
    endpoint: "https://opencode.ai/plugin/openai" 
    api_key_env: "" 
```

**Selection Logic**: The `opencode_ci_runner.py` script automatically inspects the task instruction.

- **Complex Tasks** (keyword `refactor`, `architect`, or length > 400 chars): Uses `reasoning_model` (`max`).
- **Simple Tasks**: Uses `model` (`mini`).

### Single Model Config

For simpler agents (Steward):
...
## 3. Supported Models & Performance

| Model | Role | Latency (Typical) | Use Case |
| :--- | :--- | :--- | :--- |
| `gpt-5.1-codex-mini` | Default / Steward | ~8s | Routine edits, file creation, simple logic. |
| `gpt-5.1-codex-max` | Reasoning / Architect | ~60s | Complex refactors, architecture, deeper analysis. |
| `gpt-5.2-codex` | Experimental | >120s (Timeout) | **Avoid**. Currently exhibits extreme latency with plugin. |

## 4. Behavior & Technical Details

- **Routing**: When `provider` is `opencode-openai`, execution is delegated to the local `opencode run` CLI via `scripts/opencode_ci_runner.py`.
- **Environment**: The CLI process inherits the User's environment (for plugin discovery) but runs within a Git-diff sandbox for safety.
- **Encoding**: The runner enforces UTF-8 encoding to handle unicode characters from advanced models (fixing Windows CP1252 crashes).
- **Timeouts**:
  - Standard: 300s (5 mins).
  - Max Diagnostic: 600s (10 mins).

## 5. Usage in LifeOS

This option interacts with **Phase 4 Autonomy** via the `OpenCodeClient` execution path. It enables LifeOS to utilize your personal ChatGPT subscription ("Dogfooding") instead of burning API credits.
