# Review Packet: Master Execution Plan + Phase 0 Execution v1.0

## Mission
Write superseding master plan, execute Phase 0 actions (truth alignment, deprecated path guard, targeted test fixes), and initiate doc-steward automation to reduce recurrent manual audits.

## Execution Summary
- Wrote superseding plan to artifacts/plans/LifeOS_Master_Execution_Plan_v1.0.md.
- Implemented deprecated-path guard for mission CLI and autonomous_build_cycle mission entrypoint.
- Fixed targeted recursive test failures: timeout watchdog import gap and stewardship runner interpreter assumptions.
- Updated LIFEOS_STATE/BACKLOG/INDEX and added supersession + freshness gate docs.
- Added runtime status generator script and generated artifacts/status/runtime_status.json.
- Regenerated docs/LifeOS_Strategic_Corpus.md per doc stewardship rule.
- OpenCode availability confirmed via `opencode --version` (1.0.223). OpenClaw status check in this shell returned uv_interface_addresses confinement error and was recorded as fallback evidence.

## Verification
- `pytest -q tests_recursive/test_e2e_smoke_timeout.py` => 6 passed.
- `pytest -q tests_recursive/test_steward_runner.py` => 26 passed, 1 xfailed.
- `pytest -q runtime/tests/test_cli_mission.py` => 11 passed.
- `python3 scripts/generate_runtime_status.py` => status=ok.

## Appendix A: Flattened Code (Full)

### File: artifacts/plans/LifeOS_Master_Execution_Plan_v1.0.md
```
# LifeOS Master Execution Plan v1.0 (Superseding)

## Supersession

This plan supersedes prior planning artifacts for this stream, including:
...
This is the authoritative execution plan for stateless agents until replaced.

## Ground Truth (Locked)

1. OpenClaw install and P1 acceptance are complete as of 2026-02-11.
2. LoopSpine is production-ready and CLI-wired.
3. `autonomous_build_cycle` is still callable and contains stubs; new runs must be blocked on this path.
4. Canonical runtime contracts are existing types (`MissionContext`, `MissionResult`, `BaseMission`, `AgentCall`).
5. `ReviewMission` performs real LLM calls but remains MVP-thin.
6. Budget controller stub remains at `runtime/orchestration/loop/configurable_policy.py:538`.
7. Recursive test debt remains in `tests_recursive/test_steward_runner.py` and `tests_recursive/test_e2e_smoke_timeout.py`.
8. `docs/11_admin/LIFEOS_STATE.md` and `docs/11_admin/BACKLOG.md` required reconciliation.

## Constraints and Invariants

1. Use existing contract types; do not introduce parallel contract systems in this phase.
2. Fail closed on policy/contract/validator ambiguity.
3. Keep `BACKLOG.md` as priority authority.
4. OpenClaw is orchestration surface; LoopSpine is execution controller.
5. Deprecated path guard for `autonomous_build_cycle` is required.
...
1. Update `docs/11_admin/LIFEOS_STATE.md` with OpenClaw completion.
2. Update `docs/11_admin/BACKLOG.md`: mark OpenClaw install done, promote E2E loop validation to P0.
3. Update `docs/INDEX.md` timestamp.
4. Regenerate `docs/LifeOS_Strategic_Corpus.md`.
5. Record plan supersession in `docs/11_admin/Plan_Supersession_Register.md`.

Exit criteria:

1. State docs match runtime facts.
2. No contradiction between `LIFEOS_STATE.md` and `BACKLOG.md` on OpenClaw status.

### 0B. Guard Deprecated Path

Tasks:

1. Add CLI guard in `runtime/cli.py` to block `mission run autonomous_build_cycle` with migration guidance.
2. Add mission-level guard in `runtime/orchestration/missions/autonomous_build_cycle.py` for new runs.
3. Preserve compatibility path for historical replay via explicit opt-in flag only.

Exit criteria:

1. New autonomous runs only start with `lifeos spine run`.

### 0C. Test Debt Triage and Repair

Tasks:

1. Fix `tests_recursive/test_e2e_smoke_timeout.py` import path breakage.
2. Fix `tests_recursive/test_steward_runner.py` environment-coupled `python` invocation assumptions.
3. Re-run targeted suites and record residuals.

Exit criteria:

1. Targeted suites green or formally triaged with explicit residual reason.

### 0D. Doc Freshness Gate Skeleton

Tasks:

1. Add `scripts/generate_runtime_status.py`.
2. Generate machine status artifact from runtime probes.
3. Document enforcement switch date (warning now, blocking at end of Week 2) in `docs/11_admin/Doc_Freshness_Gate_Spec_v1.0.md`.

Exit criteria:

1. Generator script and spec exist and run.

## Phase 1 (Week 1-2): OpenClaw Orchestrator Readiness

Tasks:

1. Map OpenClaw job submission -> `spine.run(task_spec)`.
2. Map Spine checkpoint/terminal output -> OpenClaw result surface.
3. Route evidence into deterministic OpenClaw job evidence paths.
4. Enforce worktree dispatch isolation.
5. Wire validator hooks pre/post dispatch with fail-closed behavior.

Exit criteria:

1. OpenClaw can dispatch a job, observe it, and close it with complete evidence.

## Phase 2 (Week 1-2, Parallel): E2E Spine Validation + Budget Hardening

Tasks:

1. Run one real backlog task end-to-end via Spine.
2. Replace budget eligibility stub and wire real token accounting.
3. Fail closed on missing token usage (`TOKEN_ACCOUNTING_UNAVAILABLE`).
4. Validate checkpoint/resume in a controlled checkpoint scenario.

Exit criteria:

1. One real autonomous run completes with checkpoint/resume and budget semantics verified.

## Phase 3 (Week 2-4): Codemoot Integration Spike

Tasks:

1. Evaluate Codemoot API/auth/output contracts.
2. Integrate Codemoot at mission executor level first.
3. Map Codemoot outputs to `MissionResult`.
4. Validate with real tickets and compare against baseline runtime path.

Exit criteria:

1. Codemoot-backed mission path runs real work with existing contracts.

## Phase 4 (Week 2-4): Stabilization + Automation

Tasks:

1. Implement ledger hash chaining (tamper-evident linking).
2. Promote doc freshness gate to blocking.
3. Finalize pending protocol/admin docs listed in backlog.
4. Auto-report documentation drift weekly.

Exit criteria:

1. Doc drift is machine-gated, reducing recurring manual audits.

## Stateless Actor Handoff Contract

Each task execution must output:

1. Files changed.
2. Tests run and outcomes.
3. Evidence artifact references.
4. Review packet path.
5. Explicit stop condition if blocked.

Stop conditions:

1. Contract violation.
2. Missing validator evidence.
3. Protected-path scope ambiguity.
4. Unresolved policy mismatch.
```
...
### File: docs/INDEX.md
```
# LifeOS Strategic Corpus [Last Updated: 2026-02-12]
...
**Active WIP:** mainline execution hardening (deprecated path guard + test debt + doc freshness skeleton)
...
```
{
  ...
  "generated_at_utc": "2026-02-12T01:20:22.513224+00:00",
