# Review Packet: Master Execution Plan v1.1 Canonicalization v1.0

## Mission
Incorporate review suggestions into canonical plan, update state/index/project status docs, and align doc-steward automation artifacts with canonical plan authority.

## Suggestions Addressed
1. Added explicit checkpoint artifact path/schema: artifacts/packets/status/checkpoint_report_<date>.json using runtime status schema.
2. Added explicit mapping module location: runtime/orchestration/openclaw_bridge.py.
3. Added upstream token-usage dependency note: runtime/agents/api.py usage zeros and OpenCodeClient parsing requirement.
4. Added Codemoot discovery gate: W6-T00 before timeline commitment.
5. Added Key Files Reference table for stateless actor pickup.

## Verification
- python3 docs/scripts/generate_strategic_context.py -> success
- python3 scripts/generate_runtime_status.py -> status=ok
- pytest -q tests_recursive/test_e2e_smoke_timeout.py runtime/tests/test_cli_mission.py -> 17 passed
...
This is the canonical execution plan for stateless agents.

## Ground Truth (Locked)

1. OpenClaw is installed and acceptance-verified (2026-02-11).
2. LoopSpine is production-ready and CLI-wired.
3. `autonomous_build_cycle` is callable and contains stubs; new runs must be blocked on this path.
4. Existing contracts are canonical: `MissionResult`, `MissionContext`, `BaseMission`, `AgentCall`.
5. `ReviewMission` is real LLM-backed (MVP-thin).
6. Budget policy has stubbed eligibility at `runtime/orchestration/loop/configurable_policy.py:538`.
7. `runtime/agents/api.py` currently surfaces usage as zeros in key paths; real token accounting requires upstream parsing from OpenCodeClient.
8. State/admin docs require freshness automation to avoid recurring manual audits.

## Principles

1. Extend existing contracts; do not create parallel core contract systems in this phase.
2. Fail closed on ambiguity, policy mismatch, or missing evidence.
3. Backlog remains canonical priority source.
4. OpenClaw is orchestration surface; Spine is execution controller.
5. Evidence-first implementation: each step produces machine-checkable artifacts.

## Ownership Split

1. CEO/OpenClaw Owner:
   - Runtime environment health and OpenClaw profile/channel configuration.
2. Builder Agent Team:
   - Repo implementation, tests, doc stewardship artifacts, and git operations.

## Phase 0 (Immediate, This Week)

### 0A. Truth alignment docs

Tasks:

1. Update `docs/11_admin/LIFEOS_STATE.md` to remove stale OpenClaw blocker language.
2. Update `docs/11_admin/BACKLOG.md` to mark OpenClaw install complete and prioritize E2E Spine verification.
3. Record active canonical plan and supersession in admin register.
4. Update `docs/INDEX.md` timestamp and links.
5. Regenerate `docs/LifeOS_Strategic_Corpus.md`.

Exit:

1. State docs match runtime truth and no internal contradictions remain.

### 0B. Path guard for new runs

Tasks:

1. Block `mission run autonomous_build_cycle` in CLI with migration guidance to `lifeos spine run`.
2. Add mission-level deprecation guard for CLI mission-run path while preserving explicit replay/test compatibility.

Exit:

1. New autonomous runs can only start through Spine.

### 0C. Targeted test repairs

Tasks:

1. Fix timeout regression test import breakage (`tests_recursive/test_e2e_smoke_timeout.py` path dependency).
2. Fix interpreter assumptions in stewardship runner tests.
3. Run targeted suites and document any residual environment-coupled xfails.

Exit:

1. Targeted suites pass or have explicit triage records.

### 0D. Runtime status generator and gate skeleton

Tasks:

1. Maintain `scripts/generate_runtime_status.py`.
2. Generate runtime checkpoint artifact at:
   - `artifacts/packets/status/checkpoint_report_<YYYYMMDD>.json`
3. Use same schema contract as `artifacts/status/runtime_status.json`:
   - `generated_at_utc`
   - `repo_root`
   - `facts`
   - `contradictions`
   - `status`
4. Wire CI contradiction check in warning mode.
5. Commit blocking switch date (end of Week 2 from activation).

Exit:

1. Daily checkpoint report generation path and schema are fixed and documented.

## Phase 1 (Week 1-2): OpenClaw Orchestrator Readiness

### 1A. Mapping implementation location

Tasks:

1. Implement mapping module at:
   - `runtime/orchestration/openclaw_bridge.py`
2. Centralize conversion logic there:
   - OpenClaw job input -> Spine run payload
   - Spine checkpoint/terminal artifacts -> OpenClaw result shape
3. Keep mapping out of ad-hoc scripts.

### 1B. Evidence routing

Tasks:

1. Route run evidence to deterministic path:
   - `artifacts/evidence/openclaw/jobs/<job_id>/`
2. Include ledger pointers, packet paths, and hash summary.

### 1C. Worktree dispatch and lifecycle

Tasks:

1. Enforce isolated worktree dispatch for governed runs.
2. Add cleanup/orphan handling and explicit failure modes.

### 1D. Fail-closed validator hooks

Tasks:

1. Pre-run checks: policy hash, protected paths, envelope constraints.
2. Post-run checks: evidence completeness, packet presence, ledger append success.
3. Fail closure if any mandatory check is absent/invalid.

Exit:

1. OpenClaw can submit -> Spine executes -> OpenClaw receives closed result with complete evidence.

## Phase 2 (Week 1-2, Parallel): E2E Spine Validation and Budget Hardening

### 2A. Real task execution

Tasks:

1. Select a low-risk real backlog task.
2. Execute full chain through Spine.
3. Capture ledger, checkpoint/terminal packets, and commit evidence.

### 2B. Budget controller connection

Tasks:

1. Replace budget eligibility stub in `configurable_policy.py`.
2. Wire token accounting using real response usage.
3. Fail closed on missing token usage (`TOKEN_ACCOUNTING_UNAVAILABLE`).
4. Fail closed on budget exceed (`BLOCKED` with budget reason).

Important upstream dependency:

1. OpenCodeClient/agent layer must parse and return real token counts.
2. `runtime/agents/api.py` currently emits zero-usage in relevant flow; this must be fixed before budget gating can be trusted.

### 2C. Checkpoint/resume proof

Tasks:

1. Trigger checkpoint case.
2. Resume using `lifeos spine resume`.
3. Verify policy-hash continuity and correct step continuation.

Exit:

1. One real autonomous task completes with budget + checkpoint semantics validated.

## Phase 3 (Week 2-4): Codemoot Integration Spike

### 3A. Discovery gate

Tasks:

1. W6-T00 gate: confirm Codemoot API/documentation access in current environment.
2. If docs/API unavailable, log blocker and re-sequence timeline before implementation commitment.

### 3B. Mission-level integration first

Tasks:

1. Integrate Codemoot at mission executor level first.
2. Keep `call_agent` transport intact in this phase.
3. Map Codemoot outputs to canonical `MissionResult`.

### 3C. Real ticket validation

Tasks:

1. Run real ticket through Codemoot-backed path.
2. Compare reliability/quality against baseline.
3. Record evidence for promotion/no-promotion decision.

Exit:

1. Codemoot decision is evidence-backed, not speculative.

## Phase 4 (Week 2-4): Stabilization and Doc Automation

### 4A. Ledger tamper-evidence

Tasks:

1. Implement hash chain linking in `runtime/orchestration/loop/ledger.py`.
2. Add tamper-detection tests.

### 4B. Doc freshness gate to blocking

Tasks:

1. Promote warning gate to blocking on committed switch date.
2. Enforce freshness SLA and contradiction checks.
3. Produce weekly drift artifact and backlog feed.

### 4C. Pending doc finalization

Tasks:

1. Finalize backlog-listed protocol/admin docs.
2. Remove TODO markers and stale placeholders.
3. Regenerate strategic corpus and update index timestamps.

Exit:

1. Recurring manual audit load is replaced by gate-driven freshness controls.

## Test Strategy (Required Per Phase)

1. Run `runtime/tests` baseline before and after each phase batch.
2. Run targeted tests for changed surfaces.
3. Add tests for:
   - deprecated-path guard behavior
   - openclaw bridge mapping
   - budget fail-closed semantics
   - doc gate contradiction detection
...
| `runtime/orchestration/openclaw_bridge.py` | Central mapping module for OpenClaw integration (new target) |
| `runtime/orchestration/missions/autonomous_build_cycle.py` | Deprecated path (guarded for new runs) |
| `runtime/cli.py` | CLI entrypoint and deprecation block |
| `runtime/orchestration/loop/configurable_policy.py` | Budget policy and eligibility |
| `runtime/orchestration/loop/ledger.py` | Ledger and hash-chain hardening target |
| `scripts/generate_runtime_status.py` | Runtime status fact generator |
| `docs/11_admin/LIFEOS_STATE.md` | Canonical state narrative |
| `docs/11_admin/BACKLOG.md` | Canonical priority queue |
| `docs/11_admin/PROJECT_STATUS_v1.0.md` | Project status snapshot (info view) |
| `docs/11_admin/Plan_Supersession_Register.md` | Canonical plan authority register |
...
5. review packet path
6. blocker reason if incomplete

Stop conditions:

1. policy mismatch
2. missing mandatory evidence
3. protected-scope ambiguity
4. unresolved runtime dependency
```
...
**Current Focus:** Phase 0 execution — reconcile state docs, guard deprecated autonomous path, run E2E Spine validation
**Active WIP:** mainline execution hardening (deprecated path guard + test debt + doc freshness skeleton), OpenClaw bridge readiness
...
2. **Deprecated path guard** (P0) — block new `autonomous_build_cycle` runs; route to Spine.
3. **test_steward_runner.py / test_e2e_smoke_timeout.py** (P1) — recursive suite repair/triage.
...
1. **Canonical Plan:** `artifacts/plans/LifeOS_Master_Execution_Plan_v1.1.md`
...
```
{
  ...
  "generated_at_utc": "2026-02-12T02:06:58.377248+00:00",
...
def main() -> int:
    ...
    output_dir = repo_root / "artifacts" / "status"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "runtime_status.json"
