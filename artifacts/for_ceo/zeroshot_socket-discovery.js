...
function getTaskSocketPath(taskId) {
  ...
  ensureSocketDir();
  const sockPath = path.join(SOCKET_DIR, `${taskId}.sock`);
  console.log(`DEBUG: Returning file path: ${sockPath}`);
  return sockPath;
}
...
function getAgentSocketPath(clusterId, agentId) {
  if (isWin) {
    return `\\\\.\\pipe\\zeroshot-${clusterId}-${agentId}`;
  }
...
async function cleanupStaleSocket(socketPath) {
  if (isWin) return false; // Named pipes are managed by kernel/process logic
