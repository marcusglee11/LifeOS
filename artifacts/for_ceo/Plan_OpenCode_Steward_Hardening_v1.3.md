...
- **JSON-Only**: The runner accepts ONLY JSON payloads matching:
  ```json
  {"files": ["path1", "path2"], "action": "create|modify|delete|archive", "instruction": "..."}
  ```
...
> **Policy Note**: Phase 2 adopts a conservative trigger: any modify/delete requires a packet to maximize auditability; optimization to >1-file threshold may be considered post-passage under governance review.
...
- Create temporary git worktree for certification.
- Assert clean state at start and end.

#### Security Tests
- `test_security_symlink_index`: Stage symlink (mode 120000), verify rejection.
- `test_security_symlink_parent`: Create filesystem symlink in parent path, verify rejection.
- `test_security_path_traversal`: Try `docs/../secrets.txt`, verify rejection.
- `test_denylist_py_file`: Try to modify `docs/example.py`, verify rejection.
- `test_denylist_foundations`: Try to modify Constitution without override, verify rejection.
- `test_delete_without_packet`: Delete single file without packet, verify rejection.
- `test_packet_schema_fail`: Create packet with ellipses, verify rejection.
- `test_packet_modify_existing`: Try to modify existing packet, verify rejection.
...
1. Compute SHA-256 of `CERTIFICATION_REPORT_v1_3.json`.
...
3. **Hash Verification**: Produce `HASH_MANIFEST.json`.

## CHANGELOG
- **v1.0 -> v1.1**: Added JSON schema, Symlinks, Delete-Gate.
- **v1.1 -> v1.2**: Added Index-level symlink check, Override mechanism, Packet Schema validator.
- **v1.2 -> v1.3**:
  ...
  - P1.2: Clarified canonical CCP location.
