# Patch Summary: OpenCode Deletion Safety v1.1
...
- **Unit Tests:** `runtime/tests/test_opencode_safety.py` verifies all PathGuard invariants including the new prefix collision case.
- **Smoke Test:** `scripts/smoke_opencode_safety.py` provides runtime verification of blocked vs. allowed scenarios.
- **Log Evidence:** Captured in `04_test_log_full.txt` and `05_smoke_log.txt`.
