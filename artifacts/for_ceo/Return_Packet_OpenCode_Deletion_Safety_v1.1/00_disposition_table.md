...
|---|---|---|---|---|
| `runtime/agents/opencode_client.py` | 343 | `shutil.rmtree` | GUARDED | Wrapped with `PathGuard.verify_safe_for_destruction`. |
| `scripts/opencode_ci_runner.py` | 288 | `shutil.rmtree` | GUARDED | Wrapped with `PathGuard.verify_safe_for_destruction`. |
...
| `scripts/run_certification_tests.py` | 346 | `shutil.rmtree` | GUARDED_BY_CONTEXT | Test runner cleans its own temp dirs. |
