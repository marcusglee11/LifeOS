...
from .util.questions import QuestionType, raise_question
...
class RuntimeFSM:
    ...
    def __init__(self, strict_mode: Optional[bool] = None):
        ...
        self._transitions: Dict[RuntimeState, List[RuntimeState]] = {
            ...
            RuntimeState.MIGRATION_SEQUENCE: [
                RuntimeState.GATES,
                RuntimeState.ERROR,
    ...
    def transition_to(self, next_state: RuntimeState) -> None:
        ...
        if next_state not in self._transitions[self.__current_state]:
            ...
            self._force_error(
                f"Invalid transition attempted: {self.__current_state} -> {next_state}"
            )
            return
    ...
    def _force_error(self, reason: str) -> None:
        ...
        try:
            raise_question(
                QuestionType.FSM_STATE_ERROR,
                f"RUNTIME HALT: {reason}. Please raise a QUESTION to the CEO.",
            )
        except Exception:
    ...
    def assert_state(self, expected_state: RuntimeState) -> None:
        ...
        if self.__current_state != expected_state:
            self._force_error(
                f"State assertion failed. Expected {expected_state}, got {self.__current_state}"
            )
    ...
    def _validate_history(self) -> None:
        ...
        for i in range(len(self._history) - 1):
            ...
            if to_state not in self._transitions.get(from_state, []):
                self._force_error(
                    f"Invalid history path: {from_state.name} → {to_state.name} not allowed"
                )
                return
