...
from project_builder.orchestrator.missions import (
    check_and_apply_backpressure,
    compute_backpressure_thresholds,
)
...
def test_backpressure_pause(db_conn):
    ...
    for i in range(51):
        db_conn.execute(
            "INSERT INTO messages (id, mission_id, status, created_at) VALUES (?, ?, ?, ?)",
    ...
    cur = db_conn.execute(
        "SELECT status, previous_status, failure_reason FROM missions WHERE id='m1'"
    )
    row = cur.fetchone()
