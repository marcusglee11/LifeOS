...
from project_builder.orchestrator.fsm import (
    start_task_execution,
    transition_to_failed_terminal,
    transition_to_repair_retry,
)
...
def test_start_task_execution(db_conn):
    ...
    cur = db_conn.execute(
        "SELECT status, repair_attempt, repair_context FROM mission_tasks WHERE id='t1'"
    )
    row = cur.fetchone()
