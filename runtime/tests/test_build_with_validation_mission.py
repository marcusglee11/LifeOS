...
from runtime.orchestration.missions.base import (  # Added MissionType for P0.2
    MissionContext,
    MissionType,
...
def test_mission_context_runtime_failures(tmp_path, mission):
    ...
    ctx_empty = MissionContext(
        repo_root=empty_repo, baseline_commit=None, run_id="test_run", metadata={}
    )
...
def context(tmp_path):
    ...
    repo.mkdir()
    (repo / "pyproject.toml").touch()
    return MissionContext(repo_root=repo, baseline_commit="a" * 40, run_id="test_run", metadata={})
...
def test_validate_inputs_valid(mission):
    valid_inputs = {"mode": "smoke", "pytest_args": ["-v"], "pytest_targets": ["foo"]}
    # Should not raise
