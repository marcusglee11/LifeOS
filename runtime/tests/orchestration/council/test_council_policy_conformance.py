...
def extract_enum_definitions(policy: dict[str, Any]) -> dict[str, set[str]]:
    ...
    for enum_name, enum_values in enums.items():
        if not isinstance(enum_values, list):
            raise ValueError(f"Enum '{enum_name}' must be a list, got {type(enum_values).__name__}")
...
def collect_enum_references(
    section: dict[str, Any], enum_fields: list[str]
) -> dict[str, list[str]]:
    """
...
def validate_references(
    references: dict[str, list[str]], definitions: dict[str, set[str]]
) -> list[str]:
    """
...
class TestModeEnumReferences:
    ...
    def test_mode_enum_references(self):
        ...
        references = collect_enum_references(
            policy["modes"], ["mode_type", "execution_model", "priority"]
        )
...
class TestSeatEnumReferences:
    ...
    def test_seat_enum_references(self):
        ...
        references = collect_enum_references(
            policy["seats"], ["role", "authority_level", "decision_scope"]
        )
...
class TestTierEnumReferences:
    ...
    def test_tier_enum_references(self):
        ...
        references = collect_enum_references(
            policy["tiers"], ["governance_level", "approval_authority", "review_scope"]
        )
...
class TestLensEnumReferences:
    ...
    def test_lens_enum_references(self):
        ...
        references = collect_enum_references(
            policy["lenses"], ["perspective_type", "analysis_depth", "focus_area"]
        )
...
class TestComprehensiveValidation:
    ...
    def test_all_enum_references_defined(self):
        ...
        for section_name, enum_fields in sections.items():
            ...
            references = collect_enum_references(policy[section_name], enum_fields)
