...
def dogfood_context(tmp_path):
    ...
    return MissionContext(
        repo_root=repo_root, baseline_commit="demo", run_id="dogfood_1", operation_executor=None
    )
        ...
        with (
            ...
            B.return_value.run.side_effect = [
                MissionResult(
                    True,
                    MissionType.BUILD,
                    outputs={"review_packet": {"payload": {"content": "diff_attempt_1"}}},
                    evidence={"usage": {"total": 1}},
                ),
                MissionResult(
                    True,
                    MissionType.BUILD,
                    outputs={"review_packet": {"payload": {"content": "diff_attempt_2"}}},
                    evidence={"usage": {"total": 1}},
                ),
            ]
            ...
            results = [
                ...
                MissionResult(
                    True,
                    MissionType.REVIEW,
                    outputs={"verdict": "rejected", "council_decision": {"synthesis": "bad code"}},
                    evidence={"usage": {"total": 1}},
                ),  # Attempt 1 Output
                MissionResult(
                    True,
                    MissionType.REVIEW,
                    outputs={"verdict": "approved", "council_decision": {"synthesis": "good code"}},
                    evidence={"usage": {"total": 1}},
                ),  # Attempt 2 Output
            ]
            ...
            S.return_value.run.return_value = MissionResult(
                True,
                MissionType.STEWARD,
                outputs={"commit_hash": "final_hash"},
                evidence={"usage": {"total": 1}},
            )
