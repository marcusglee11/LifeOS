...
from runtime.orchestration.loop.taxonomy import TerminalReason
...
def acceptance_context(tmp_path):
    ...
    return MissionContext(
        repo_root=repo_root, baseline_commit="abc", run_id="acc_run", operation_executor=None
    )
...
def mock_subs():
    with (
        patch("runtime.orchestration.missions.autonomous_build_cycle.DesignMission") as D,
...
def mock_review_behavior(ctx, inputs):
    ...
    return MissionResult(
        True,
        MissionType.REVIEW,
        outputs={"verdict": "rejected", "council_decision": {"synthesis": "rejected"}},
        evidence={"usage": {"total": 1}},
    )
...
def test_crash_and_resume(acceptance_context, mock_subs):
    ...
    B.return_value.run.side_effect = [
        MissionResult(
            True,
            MissionType.BUILD,
            outputs={"review_packet": {"payload": {"content": "diff1"}}},
            evidence={"usage": {"total": 1}},
        ),
    ...
    B.return_value.run.return_value = MissionResult(
        True,
        MissionType.BUILD,
        outputs={"review_packet": {"payload": {"content": "diff2"}}},
        evidence={"usage": {"total": 1}},
    )
    ...
    S.return_value.run.return_value = MissionResult(
        True, MissionType.STEWARD, outputs={"commit_hash": "xyz"}, evidence={"usage": {"total": 1}}
    )
...
def test_acceptance_oscillation(acceptance_context, mock_subs):
    ...
    B.return_value.run.side_effect = [
        MissionResult(
            True,
            MissionType.BUILD,
            outputs={"review_packet": {"payload": {"content": "diffA"}}},
            evidence={"usage": {"total": 1}},
        ),
        MissionResult(
            True,
            MissionType.BUILD,
            outputs={"review_packet": {"payload": {"content": "diffB"}}},
            evidence={"usage": {"total": 1}},
        ),
        MissionResult(
            True,
...
def test_verify_terminal_packet_structure(acceptance_context, mock_subs):
    ...
    B.return_value.run.return_value = MissionResult(
        True,
        MissionType.BUILD,
        outputs={"review_packet": {"payload": {"content": "u"}}},
        evidence={"usage": {"total": 1}},
    )
    ...
    with patch(
        "runtime.orchestration.missions.autonomous_build_cycle.BudgetController"
    ) as MockBudget:
        MockBudget.return_value.check_budget.return_value = (
            True,
            TerminalReason.BUDGET_EXHAUSTED.value,
        )
...
def test_diff_budget_exceeded(acceptance_context, mock_subs):
    ...
    B.return_value.run.return_value = MissionResult(
        True,
        MissionType.BUILD,
        outputs={"review_packet": {"payload": {"content": massive_diff}}},
        evidence={"usage": {"total": 1}},
    )
