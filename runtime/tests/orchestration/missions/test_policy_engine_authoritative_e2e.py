...
from runtime.orchestration.loop.policy import EscalationArtifact, LoopPolicy
from runtime.tools.registry import get_registry, reset_global_registry
from runtime.tools.schemas import ToolErrorType, ToolInvokeRequest
...
def mock_ledger_with_attempts(tmp_path):
    ...
    header = LedgerHeader(
        policy_hash="test_policy_v1", handoff_hash="test_handoff", run_id="test_run_123"
    )
...
def test_e2e_1_authoritative_on_uses_policy_engine(
    policy_config_dir_with_loop_rules, tmp_path, monkeypatch
):
    ...
    monkeypatch.setenv(
        "LIFEOS_WORKSPACE_ROOT", str(policy_config_dir_with_loop_rules.parent.parent)
    )
    ...
    assert policy._config_policy is not None, (
        "Config-driven policy should be instantiated when failure_routing present"
    )
    assert isinstance(policy._config_policy, ConfigurableLoopPolicy), (
        "_config_policy should be ConfigurableLoopPolicy instance"
    )
    ...
    with patch.object(
        policy, "_hardcoded_decide", wraps=policy._hardcoded_decide
    ) as mock_hardcoded:
        # Also spy on config-driven policy to prove it IS called
        with patch.object(
            policy._config_policy,
            "decide_next_action",
            wraps=policy._config_policy.decide_next_action,
        ) as mock_config:
            action, reason = policy.decide_next_action(ledger)

            # Assert: _hardcoded_decide was NOT called
            assert not mock_hardcoded.called, (
                "_hardcoded_decide should NOT be called when config-driven policy is active"
            )
...
def test_e2e_5_escalation_artifact_determinism(
    tmp_path, policy_config_dir, mock_ledger_with_attempts, monkeypatch
):
...
def test_e2e_5_unresolvable_escalation_write_fails_closed(tmp_path, monkeypatch):
    ...
    with patch("builtins.open", side_effect=mock_open):
        ...
        with pytest.raises(PermissionError):
            EscalationArtifact.write(reason="Test escalation", requested_authority="CEO")
...
def test_e2e_6a_valid_waiver_resumes(tmp_path, mock_ledger_with_attempts, monkeypatch):
    ...
    waiver_context = {"failure_class": "REVIEW_REJECTION", "retry_count": 3, "retry_limit": 3}
