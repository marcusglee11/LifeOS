...
def mock_context(tmp_path):
    ...
    return MissionContext(
        repo_root=repo_root, baseline_commit="abc", run_id="test_run", operation_executor=None
    )
...
def mock_sub_missions():
    with (
        patch("runtime.orchestration.missions.autonomous_build_cycle.DesignMission") as MockDesign,
        ...
        patch(
            "runtime.orchestration.missions.autonomous_build_cycle.StewardMission"
        ) as MockSteward,
    ):
        ...
        b_inst.run.return_value = MissionResult(
            True,
            MissionType.BUILD,
...
def test_token_accounting_fail_closed(mock_context, mock_sub_missions):
    ...
    b_inst = MockBuild.return_value
    b_inst.run.return_value = MissionResult(
        True, MissionType.BUILD, outputs={"review_packet": {}}, evidence={}
    )  # Missing usage
...
def test_budget_exhausted(mock_context, mock_sub_missions):
    ...
    b_inst.run.side_effect = [
        MissionResult(
            True,
            MissionType.BUILD,
            outputs={"review_packet": {"payload": {"content": f"diff_{i}"}}},
            evidence={"usage": {"total": 1}},
        )
        for i in range(10)
    ]
