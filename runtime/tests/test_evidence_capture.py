...
from runtime.tools.evidence_capture import CaptureStatus, run_command_capture
...
def test_successful_command_capture(tmp_path):
    ...
    cmd = [
        sys.executable,
        "-c",
        "import sys; print('hello stdout'); print('hello stderr', file=sys.stderr); sys.exit(0)",
    ]

    result = run_command_capture(
        step_name="success_test", cmd=cmd, cwd=tmp_path, evidence_dir=evidence_dir
    )
...
def test_unicode_bytes_preserved(tmp_path):
    ...
    cmd = [
        sys.executable,
        "-c",
        "import sys; sys.stdout.buffer.write(b'\\xe2\\x9d\\xa4'); sys.stdout.buffer.flush()",
    ]
...
def test_large_output_streaming(tmp_path):
    ...
    cmd = [
        sys.executable,
        "-c",
        f"import sys; sys.stdout.buffer.write(b'A' * {size_mb} * 1024 * 1024)",
    ]
