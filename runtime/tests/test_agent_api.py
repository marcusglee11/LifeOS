...
class TestCanonicalJson:
    ...
    def test_rejects_nan_fail_closed(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            canonical_json(obj)
        assert (
            "Out of range float values" in str(exc_info.value)
            or "NaN" in str(exc_info.value).upper()
        )
    ...
    def test_rejects_infinity_fail_closed(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            canonical_json(obj)
        assert "Out of range float values" in str(exc_info.value) or "Infinity" in str(
            exc_info.value
        )
...
class TestDeterministicIds:
    ...
    def test_run_id_changes_with_different_inputs(self):
        ...
        id1 = compute_run_id_deterministic(mission_spec, "hash1", {"f": "h1"}, code_version)
        id2 = compute_run_id_deterministic(mission_spec, "hash2", {"f": "h1"}, code_version)
