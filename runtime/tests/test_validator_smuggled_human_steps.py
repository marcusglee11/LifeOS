...
from runtime.validator import AntiFailureValidator, StepActor, WorkflowStep
...
class TestValidatorSmuggledHumanSteps(unittest.TestCase):
    ...
    def test_detect_manual_in_agent_step(self):
        ...
        self.assertTrue(
            any("smuggled" in w.lower() or "manual" in w.lower() for w in result.warnings)
        )
    ...
    def test_detect_user_must_in_agent(self):
        ...
        steps = [
            WorkflowStep(
                name="Deploy", actor=StepActor.AGENT, description="User must confirm deployment"
            )
        ]
