...
from project_builder.orchestrator.reclaim import WorkerRegistry, attempt_reclaim_task
...
def test_reclaim_success(db_conn):
    ...
    # Verify event
    cur = db_conn.execute(
        "SELECT event_type FROM timeline_events WHERE task_id='t1' AND event_type='task_reclaimed'"
    )
    assert cur.fetchone()
