...
class TestMigration(unittest.TestCase):
    ...
    def test_migration_execution(self):
        ...
        # Execute
        with (
            patch("coo_runtime.util.amu0_utils.resolve_amu0_path", return_value=self.amu0_path),
            patch(
                "coo_runtime.runtime.migration.amu0_utils.resolve_amu0_path",
                return_value=self.amu0_path,
            ),
            patch(
                "coo_runtime.runtime.rollback.amu0_utils.resolve_amu0_path",
                return_value=self.amu0_path,
            ),
            ...
            if mock_rollback.called:
                ...
                with open("debug_error.txt", "w") as f:
                    f.write(
                        f"DEBUG: Migration failed with: {self.migration.logger.error.call_args}"
                    )
