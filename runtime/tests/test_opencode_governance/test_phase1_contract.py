...
def test_t1_service_boots_smoke():
    ...
    req = {"version": "1.0", "request_id": "test-t1", "payload": {"key": "value"}}
    resp = invoke(req)
...
def test_t2_rejects_invalid_version():
    ...
    req = {"version": "0.9", "request_id": "test-t2", "payload": {}}
    resp = invoke(req)
    assert resp["status"] == "ERROR"
...
def test_t2_rejects_missing_payload():
    ...
    req = {"version": "1.0", "request_id": "test-t2"}
    resp = invoke(req)
    assert resp["status"] == "ERROR"
...
def test_t2_rejects_invalid_payload_type():
    ...
    req = {"version": "1.0", "request_id": "test-t2-type", "payload": "not-a-dict"}
    resp = invoke(req)
    assert resp["status"] == "ERROR"
    assert resp["error"]["code"] == INVALID_PAYLOAD
...
def test_t2b_internal_error_guard():
    ...
    try:
        ...
        req = {"version": "1.0", "request_id": "test-guard", "payload": {"a": 1}}
        resp = invoke(req)
        assert resp["status"] == "ERROR"
...
def test_t3_determinism():
    ...
    req = {"version": "1.0", "request_id": "test-t3", "payload": {"a": 1, "b": 2}}
    resp1 = invoke(req)
    resp2 = invoke(req)
...
def test_t4_steward_opencode_validate(tmp_path):
    ...
    cmd = [sys.executable, "-m", "doc_steward.cli", "opencode-validate", str(tmp_path)]
