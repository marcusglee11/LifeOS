...
def test_call_agent_records_receipt(tmp_path) -> None:
    ...
    with patch("runtime.agents.api._load_role_prompt", return_value=("system", "sha256:prompt")), \
...
def test_call_agent_cli_records_receipt(tmp_path) -> None:
    ...
    with patch("runtime.agents.api._load_role_prompt", return_value=("system", "sha256:prompt")), \
         ...
         patch(
             ...
             return_value=CLIProviderConfig(binary="codex", timeout_seconds=5, sandbox=True, enabled=True),
         ), \
         patch(
