...
def _run_doctor(repo_dir: Path, env: dict[str, str], *extra: str) -> subprocess.CompletedProcess[str]:
        ...
        "reasons": {
            "gateway_probe_failed": {
                ...
                "remediation": {
                    ...
                    "manual_hint": "Gateway probe failed. Run coo doctor --apply-safe-fixes to attempt gateway restart, or run coo stop && coo start manually.",
                },
            },
            ...
            "cron_delivery_guard_failed": {
                ...
                "remediation": {
                    ...
                    "action_id": None,
                    "fix_command": None,
                    "manual_hint": "Cron delivery policy violation. Check openclaw cron list --all --json and set delivery.mode per policy.",
                },
            },
            "sandbox_mode_disallowed": {
                ...
                "remediation": {
                    ...
                    "action_id": None,
                    "fix_command": None,
                    "manual_hint": "Adjust sandbox policy; contact the operator if the active posture is unexpected.",
                },
            },
        },
    }
...
def test_doctor_gateway_probe_failed_emits_safe_fix_command(tmp_path: Path) -> None:
    ...
    env["STUB_GATE_STATUS_JSON"] = json.dumps({"pass": False, "blocking_reasons": ["gateway_probe_failed"]})
...
def test_doctor_model_ladder_policy_failed_emits_coo_models_fix(tmp_path: Path) -> None:
    ...
    _write_doctor_catalog(repo_dir)
    env["STUB_VERIFY_RC"] = "1"
    env["STUB_GATE_STATUS_JSON"] = json.dumps({"pass": False, "blocking_reasons": ["model_ladder_policy_failed"]})
...
def test_doctor_cron_delivery_guard_failed_is_manual(tmp_path: Path) -> None:
    ...
    _write_doctor_catalog(repo_dir)
    env["STUB_VERIFY_RC"] = "1"
    env["STUB_GATE_STATUS_JSON"] = json.dumps({"pass": False, "blocking_reasons": ["cron_delivery_guard_failed"]})
...
def test_doctor_sandbox_hard_failure_is_manual(tmp_path: Path) -> None:
    ...
    _write_doctor_catalog(repo_dir)
    env["STUB_VERIFY_RC"] = "1"
    env["STUB_GATE_STATUS_JSON"] = json.dumps({"pass": False, "blocking_reasons": ["sandbox_mode_disallowed"]})
...
def test_doctor_mixed_hard_and_drift_classify_correctly(tmp_path: Path) -> None:
    ...
    env["STUB_GATE_STATUS_JSON"] = json.dumps(
        {"pass": False, "blocking_reasons": ["sandbox_mode_disallowed", "model_ladder_policy_failed"]}
    )
