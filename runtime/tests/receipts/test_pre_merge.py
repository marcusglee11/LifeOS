...
def test_built_receipt_carries_policy_version(
    ...
    receipt = build_acceptance_receipt(
        ...
        sample_workspace_tree_oid,
        sample_plan_core_sha256,
        emitter,
        decision,
        {"overall_status": "PASS"},
    )
...
def test_pre_merge_result_allowed_fields():
    ...
    r = PreMergeResult(
        allowed=True, reason_code="ACCEPTED", receipt={"receipt_id": "X"}, detail="ok"
    )
...
def test_allowed_when_active_accepted_receipt_exists(
    ...
    from runtime.receipts.pre_merge import RC_ACCEPTED, run_pre_merge_check

    _make_store_with_receipt(
...
def test_blocked_when_no_receipt_exists(tmp_store, sample_workspace_sha, sample_plan_core_sha256):
    from runtime.receipts.pre_merge import RC_NO_RECEIPT, run_pre_merge_check
    from runtime.receipts.store import ReceiptStore
...
def test_blocked_when_decision_is_rejected(
    ...
    monkeypatch.setattr(pc, "resolve_tree_oid", lambda sha, **kw: sample_workspace_tree_oid)
    from runtime.receipts.pre_merge import RC_DECISION_NOT_ACCEPTED, run_pre_merge_check
...
def test_blocked_when_missing_policy_version(
    ...
    monkeypatch.setattr(pc, "resolve_tree_oid", lambda sha, **kw: sample_workspace_tree_oid)
    from runtime.receipts.pre_merge import RC_MISSING_POLICY_VERSION, run_pre_merge_check
    from runtime.receipts.receipt_emitter import build_acceptance_receipt
    from runtime.receipts.runlog import RunLogEmitter
    from runtime.receipts.store import ReceiptStore
...
def test_blocked_when_tree_oid_mismatches(
    ...
    from runtime.receipts.pre_merge import RC_TREE_OID_MISMATCH, run_pre_merge_check
...
def test_fail_closed_when_store_root_missing(
    ...
    from runtime.receipts.pre_merge import RC_STORE_ERROR, run_pre_merge_check

    result = run_pre_merge_check(
        sample_workspace_sha, sample_plan_core_sha256, tmp_path / "no_such_store"
    )
    assert result.allowed is False
...
def test_fail_closed_on_resolve_tree_oid_error(
    ...
    monkeypatch.setattr(
        pc,
        "resolve_tree_oid",
        lambda sha, **kw: (_ for _ in ()).throw(ValueError("git unavailable")),
    )
    from runtime.receipts.pre_merge import RC_STORE_ERROR, run_pre_merge_check
...
def test_result_includes_receipt_on_allow(
    ...
    _, receipt = _make_store_with_receipt(
        tmp_store, sample_workspace_sha, sample_workspace_tree_oid, sample_plan_core_sha256
    )
    result = run_pre_merge_check(sample_workspace_sha, sample_plan_core_sha256, tmp_store)
    assert result.receipt is not None
...
def test_result_detail_nonempty_on_block(tmp_store, sample_workspace_sha, sample_plan_core_sha256):
    from runtime.receipts.pre_merge import run_pre_merge_check
    from runtime.receipts.store import ReceiptStore
