...
from project_builder.sandbox.manifest import ManifestValidationError as SandboxManifestError
...
def test_malformed_checksum():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = [{"path": "test.txt", "checksum": "1234567890abcdef"}]
        (root / ".coo-manifest.json").write_text(json.dumps(manifest))
...
def test_valid_paths_with_spaces():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = [{"path": "My Document.txt", "checksum": "sha256:" + "0" * 64}]
        (root / ".coo-manifest.json").write_text(json.dumps(manifest))
...
def test_checksum_line_ending_normalization():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = [{"path": "test.txt", "checksum": f"sha256:{expected_hash}"}]
        (root / ".coo-manifest.json").write_text(json.dumps(manifest))
