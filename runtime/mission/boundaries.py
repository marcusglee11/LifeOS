...
def validate_mission_id(
    ...
    if len(mid.value) > config.max_id_chars:
        raise MissionBoundaryViolation(f"Mission ID exceeds max length of {config.max_id_chars}")
...
def validate_mission_definition(
    ...
    Note:
        Metadata is expected to be canonicalized upstream into tuple-of-tuples.
        This validator enforces canonical form.
    """
    if config is None:
        config = MissionBoundaryConfig()
    ...
    if not isinstance(defn.tags, tuple):
        raise MissionBoundaryViolation(f"Tags must be tuple, got {type(defn.tags).__name__}")

    # Tags: count check
    if len(defn.tags) > config.max_tags:
        raise MissionBoundaryViolation(f"Too many tags: {len(defn.tags)} > {config.max_tags}")
    ...
    for i, tag in enumerate(defn.tags):
        ...
        if not tag or len(tag.strip()) == 0:
            raise MissionBoundaryViolation(f"Tag[{i}] must not be empty or whitespace-only")
    ...
    for i, pair in enumerate(defn.metadata):
        if not isinstance(pair, tuple) or len(pair) != 2:
            raise MissionBoundaryViolation(f"Metadata[{i}] must be (key, value) tuple")
        key, value = pair
