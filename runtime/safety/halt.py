...
def log_halt_event(lineage, timestamp: str, reason: str, triggered_by: str = "system") -> None:
    """
...
def find_last_good_snapshot(amu0_root: str) -> Optional[str]:
    ...
    snapshots = sorted(
        [d for d in snapshots_dir.iterdir() if d.is_dir()], key=lambda x: x.name, reverse=True
    )
...
def rollback_to_snapshot(current_state_path: str, snapshot_path: str) -> bool:
    """
...
def halt_runtime(
    ...
    if amu0_root and current_state_path:
        ...
        if snapshot:
            ...
            rollback_performed = rollback_to_snapshot(current_state_path, snapshot)
