...
def check_dap_write_health(dap_gateway, test_path: str) -> HealthStatus:
    """
    ...
    try:
        ...
        return HealthStatus(ok=True, component="DAP", reason="DAP gateway validation successful")
    except Exception as e:
        return HealthStatus(ok=False, component="DAP", reason=f"DAP gateway validation failed: {e}")


def check_index_coherence(index_updater) -> HealthStatus:
    """
    ...
    try:
        ...
        if is_coherent:
            return HealthStatus(
                ok=True, component="INDEX", reason="INDEX is coherent with directory contents"
            )
        else:
            return HealthStatus(
                ok=False,
                ...
                details={"missing_from_index": missing, "orphaned_in_index": orphaned},
            )
    except Exception as e:
        return HealthStatus(
            ok=False, component="INDEX", reason=f"INDEX coherence check failed: {e}"
        )


def check_amu0_readability(lineage_path: str) -> HealthStatus:
    """
    ...
    try:
        ...
        if not path.exists():
            return HealthStatus(
                ok=False, component="AMU0", reason=f"AMU₀ lineage file not found: {lineage_path}"
            )
        ...
        if "entries" not in data:
            return HealthStatus(
                ok=False, component="AMU0", reason="AMU₀ lineage missing 'entries' key"
            )
        ...
        return HealthStatus(
            ok=False, component="AMU0", reason=f"AMU₀ lineage JSON parse error: {e}"
        )
    except Exception as e:
        return HealthStatus(
            ok=False, component="AMU0", reason=f"AMU₀ readability check failed: {e}"
        )
...
def check_amu0_chain_integrity(lineage) -> HealthStatus:
    ...
    try:
        ...
        if is_valid:
            return HealthStatus(ok=True, component="AMU0_CHAIN", reason="AMU₀ hash chain is valid")
        else:
            return HealthStatus(
                ok=False,
        ...
        return HealthStatus(
            ok=False, component="AMU0_CHAIN", reason=f"AMU₀ chain verification failed: {e}"
        )
