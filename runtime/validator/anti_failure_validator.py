...
class AntiFailureValidator:
    ...
    def __init__(
        self, max_steps: int = 5, max_human_steps: int = 2, detect_adversarial: bool = True
    ):
        """
        ...
        self._compiled_patterns = [re.compile(p, re.IGNORECASE) for p in self.HIDDEN_HUMAN_PATTERNS]
    ...
    def validate(self, steps: List[WorkflowStep]) -> ValidationResult:
        ...
        if routine_human:
            for step in routine_human:
                violations.append(f"Routine human operation not allowed: '{step.name}'")
        ...
        if self.detect_adversarial:
            ...
            for step_name, pattern in smuggled:
                warnings.append(
                    f"Potential smuggled human effort in '{step_name}': matches pattern '{pattern}'"
                )
    ...
    def _detect_smuggled_human_steps(self, steps: List[WorkflowStep]) -> List[tuple]:
        """
    ...
    def check_workflow_chaining(
        ...
        total_human = sum(sum(1 for s in wf if s.actor == StepActor.HUMAN) for wf in workflows)
