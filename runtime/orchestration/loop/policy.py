...
from .taxonomy import FailureClass, LoopAction, TerminalOutcome, TerminalReason
...
class ConfigDrivenLoopPolicy:
    ...
    def __init__(
        self, loop_rules: List[Dict[str, Any]], escalation_config: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def _handle_escalate(self, rule: Dict[str, Any], failure_class: str) -> Tuple[str, str]:
        ...
        if not allowlist and bootstrap_mode:
            ...
            return (
                LoopAction.ESCALATE.value,
                f"{TerminalOutcome.ESCALATION_REQUESTED.value}:artifact={artifact_path}",
            )
