...
class ConfigurableLoopPolicy:
    ...
    def decide_next_action(
        self, ledger: AttemptLedger, now: Optional[datetime] = None
        ...
        if retry_count >= retry_limit:
            ...
            if self._check_escalation_triggers(ledger):
                return (
                    "terminate",
                    "Escalation triggered: protected path touched",
            ...
            if self._check_waiver_eligibility(failure_class_normalized):
                ...
                waiver_context = {
                    ...
                    if isinstance(failure_class, FailureClass)
                    else str(failure_class_str_raw).upper(),
                ...
                if waiver_artifact.check_waiver_for_context(waiver_context, now=now):
                    ...
                    return (
                        "retry",
                        f"Waiver applied for {failure_class_normalized} - resuming",
                        "WAIVER_APPLIED",
                    )
                ...
                return (
                    "terminate",
                    f"Retry limit exhausted ({retry_count}/{retry_limit}): waiver requested",
                    "WAIVER_REQUESTED",
                )
    ...
    def _count_retries_for_class(self, ledger: AttemptLedger, failure_class: FailureClass) -> int:
        ...
        target_class_upper = (
            failure_class.value.upper()
            if isinstance(failure_class, FailureClass)
            else str(failure_class).upper()
        )
        ...
        *,
        ...
        if decision["scope"].get("total_line_delta", 0) > max_lines:
            decision["decision_reason"] = (
                f"Scope: lines {decision['scope']['total_line_delta']} > max_lines {max_lines}"
            )
            return decision

        if decision["scope"].get("files_touched", 0) > max_files:
            decision["decision_reason"] = (
                f"Scope: files {decision['scope']['files_touched']} > max_files {max_files}"
            )
            return decision
        ...
        if bypass_global_count >= global_limit:
            decision["decision_reason"] = (
                f"Global bypass budget exhausted: {bypass_global_count}/{global_limit}"
            )
            return decision
