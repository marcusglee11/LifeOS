...
def _build_phase3_mission_workflow(
    mission_type: str, params: Optional[Dict[str, Any]] = None
) -> WorkflowDefinition:
    """
...
def _build_autonomous_build_cycle_workflow(
...
def _build_build_with_validation_workflow(
    params: Optional[Dict[str, Any]] = None,
) -> WorkflowDefinition:
...
def run_mission(
    ...
    if name not in MISSION_REGISTRY:
        raise UnknownMissionError(
            f"Unknown mission: '{name}'. Available missions: {sorted(MISSION_REGISTRY.keys())}"
        )
