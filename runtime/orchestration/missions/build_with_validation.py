...
from runtime.tools.evidence_capture import CaptureResult, run_command_capture
...
class BuildWithValidationMission(BaseMission):
    ...
    def run(self, context: MissionContext, inputs: Dict[str, Any]) -> MissionResult:
        ...
        evidence_dir = (
            context.repo_root / params["capture_root_rel"] / "build_with_validation" / run_token
        )
        ...
        smoke_cmd = [
            sys.executable,
            "-c",
            "import sys, os; sys.exit(0 if os.path.exists('pyproject.toml') else 1)",
        ]
        ...
        if smoke1_res.exit_code != 0:
            ...
            smoke2_res = run_command_capture(
                "smoke_compile", smoke2_cmd, context.repo_root, evidence_dir
            )
            ...
            if final_smoke.exit_code == 0:
                ...
                if not targets:
                    ...
                    partial_outputs = {
                        ...
                        "params_canonical_sha256": hashlib.sha256(
                            canonical_params_json.encode("utf-8")
                        ).hexdigest(),
