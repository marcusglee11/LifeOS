...
class DocVerifier:
    ...
    def verify_with_proposed_changes(
        self, files_modified: List[Dict], proposed_diffs: str, constraints: Dict[str, Any] = None
    ) -> VerifierOutcome:
        """
        ...
        forbidden_paths = constraints.get(
            "forbidden_paths", ["docs/00_foundations/", "docs/01_governance/", "GEMINI.md"]
        )
        ...
        if any(f.severity == "ERROR" and f.category == "BOUNDARY_VIOLATION" for f in findings):
            error_count = sum(1 for f in findings if f.severity == "ERROR")
            return VerifierOutcome(
                False, findings, f"{error_count} boundary violations", {"error_count": error_count}
            )
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if res.returncode != 0:
                    ...
                    findings.append(
                        Finding("INFO", "PATCH_APPLICATION", "Patch applied successfully")
                    )
...
def main():
    ...
    parser.add_argument("--docs-dir", type=str, default=None, help="Path to docs directory")
