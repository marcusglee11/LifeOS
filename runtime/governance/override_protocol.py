...
class OverrideProtocol:
    ...
    def apply_override(self, request: OverrideRequest, council_key: str) -> bool:
        """
        ...
        if not self._validate_council_key(council_key):
            raise OverrideProtocolError("Council authorization failed. Override not applied.")

        # Validate human approval is present
        if not request.human_approval:
            raise OverrideProtocolError("Human approval required for governance override.")
    ...
    def _append_to_log(self, request: OverrideRequest) -> None:
        ...
        log.append({"request": request.to_dict(), "request_hash": request.compute_hash()})
