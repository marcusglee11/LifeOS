...
def resolve_sandbox_root() -> Path:
    ...
    if not raw:
        raise GovernanceUnavailable("LIFEOS_SANDBOX_ROOT environment variable not set")

    raw_path = Path(raw)

    # Check if raw path exists before resolving
    if not raw_path.exists():
        raise GovernanceUnavailable(f"Sandbox root does not exist: {raw}")
    ...
    if _has_symlink_in_path(raw_path):
        raise GovernanceUnavailable(f"Sandbox root path contains symlink (denied): {raw}")
...
def check_path_scope(
    target_path: Path, scope: str, request_path: Optional[str] = None
) -> Tuple[bool, str]:
    """
